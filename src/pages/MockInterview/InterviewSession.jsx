...
export default function InterviewSession({
  ...
  const startInterview = useCallback(() => {
    ...
    const greeting = {
      type: "interviewer",
      content: `Hello! Welcome to your mock interview. We'll go through ${questions.length} questions, focusing on ${improvementAreas.join(
        ", "
      )}. Let's begin.`,
      timestamp: new Date().toLocaleTimeString(),
    };
  ...
  const nextQuestion = useCallback(() => {
    ...
    if (currentQuestionIndexRef.current < questions.length - 1) {
      ...
      // Last question -> end interview automatically
      setConversation((prev) => [
        ...prev,
        {
          type: "interviewer",
  ...
  const endInterview = useCallback(() => {
    onEndInterview?.(conversation);
  }, [conversation, onEndInterview]);
    ...
    <div style={{ width: "100%", height: "100vh", display: "flex", flexDirection: "column", background: "#f8fafc" }}>
      ...
      <div style={{ padding: "14px 22px", borderBottom: "1px solid #e2e8f0", background: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setShowCamera((s) => !s)} style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid #d1d5db", background: showCamera ? "#2563eb" : "#fff", color: showCamera ? "#fff" : "#111827", fontWeight: 600 }}>
            {showCamera ? "Hide Camera" : "Show Camera"}
          </button>
          <button onClick={onReset} style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid #d1d5db", background: "#fff", color: "#b91c1c", fontWeight: 600 }}>
            End Interview
          </button>
        </div>
      </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 28px", background: "#fff" }}>
          {!isInterviewStarted ? (
            <div style={{ display: "grid", placeItems: "center", height: "100%" }}>
              <button onClick={startInterview} style={{ padding: "14px 22px", fontSize: 18, fontWeight: 700, background: "#2563eb", color: "#fff", border: "none", borderRadius: 12 }}>
                Start Interview
              </button>
            </div>
          ) : (
            <>
              {conversation.map((m, i) => (
                <div key={i} style={{ marginBottom: 18, lineHeight: 1.6 }}>
                  <div style={{ fontSize: 11, color: "#9ca3af" }}>{m.timestamp}{m.company ? ` · ${m.company}` : ""}</div>
      ...
      {/* Controls */}
      <div style={{ position: "sticky", bottom: 0, background: "rgba(248,250,252,0.9)", backdropFilter: "saturate(180%) blur(8px)", borderTop: "1px solid #e2e8f0", padding: "12px 16px", display: "flex", alignItems: "flex-start", gap: 16, marginBottom: "20px" }}>
        ...
        <div style={{ flex: 1.2, maxWidth: 720 }}>
          <SpeechRecognition onAnswer={handleUserAnswer} isListening={isWaitingForAnswer} disabled={isProcessingAnswer} />
        </div>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          ...
          <button
            ...
            style={{ padding: "8px 10px", borderRadius: 10, border: "1px solid #d1d5db", background: "#fff", color: "#b91c1c", fontWeight: 600 }}
          >
            End
          </button>
        </div>
